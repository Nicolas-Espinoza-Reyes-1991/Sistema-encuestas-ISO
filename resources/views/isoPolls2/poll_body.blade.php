...
<div class="container mb-5">
    <div class="row mt-5 mb-3">
        <div class="col-lg-12 text-center text-primary">
            <h3 class="text-black">Formulario N°2 encuesta ISO:</h3>
        </div>
...
<script>
    $(document).ready(function () {
        ...
        $.post('{{asset("contenFormIsoPoll2")}}',{tokenEncript:tokenEncript,timer:timer,id_user:id_user,"_token": "{{ csrf_token() }}"},function(response){ 
    ...
    function saveAndEditGetMethod(validator,identifier,id,typeSave){
        ...
        $.post('{{asset("guardarEditarIsopolls2")}}',{formData:formData,identifier:identifier,id:id,typeSave:typeSave,"_token": "{{ csrf_token() }}" },function(response){ 
            if (response==1) {
                window.location.href='{{url("/homePolls")}}'+'/'+'{{$tokenEncript}}';
            }else if(response==2){
