...
<div class="container mb-5">
    <div class="row mt-5 mb-3">
        ...
        <div class="col-lg-12 text-center text-primary">
            <h3 class="text-black">Datos e Información:</h3>
        </div>
...
<script>
    $(document).ready(function () {
        ...
        id_user='{{$id_user}}';
        $.post('{{asset("contenFormPoll2")}}',{tokenEncript:tokenEncript,id_user:id_user,"_token": "{{ csrf_token() }}"},function(response){ 
    ...
    function saveAndEditGetMethod(validator,identifier,id,typeSave){
        ...
        $.post('{{asset("guardarEditarpolls2")}}',{formData:formData,identifier:identifier,id:id,typeSave:typeSave,"_token": "{{ csrf_token() }}" },function(response){ 
            if (response==1) {
                // window.location.href="{{url('/close')}}";
                window.location.href='{{url("/poll_body3")}}'+'/'+'{{$tokenEncript}}';
            }else if(response==2){
