...
class IsoPollsController4 extends Controller
{
    use genericClass;

    public function isoPoll4_body($tokenEncript)
    {
        try {
        ...
        if ($confirmadato != null || $confirmadato != '') {
            ...
            $confirmaTimer = $this->selectFirstWhere($tabla2, 'id_usuario', $id_user, 'tipo_encuesta_iso', 4);
            if ($confirmaTimer) {
                ...
                $confirmaTimer2 = $this->selectFirstWhere($tabla2, 'id_usuario', $id_user, 'tipo_encuesta_iso', 4);
                $timer = $confirmaTimer2->time;
            }
            $view = View::make('isoPolls4.poll_body', compact('tokenEncript', 'id_user', 'timer'));
            return $view;
        }
    }


    public function contenFormIsoPoll4(request $request)
    {
        $tokenEncript = $request->input('tokenEncript');
        $id_user = $request->input('id_user');
        $timer = $request->input('timer');
        $Selectores = $this->formularioEncuesta('matriz_iso4', '1');
        $SelectoresDato = $this->formularioEncuesta('matriz_iso4', '0');
        $selectGet = $this->selectFirst('encuesta_iso4', 'id_usuario', $id_user);
        $view = View::make('isoPolls4.poll_body_form', compact('tokenEncript', 'Selectores', 'SelectoresDato', 'id_user', 'selectGet', 'timer'));
