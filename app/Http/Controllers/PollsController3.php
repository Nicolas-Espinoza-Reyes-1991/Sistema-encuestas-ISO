...
class PollsController3 extends Controller
{
    use genericClass;
    ...
    public function poll_body($tokenEncript){
        ...
        if ($confirmadato != null || $confirmadato != '') { 
            $view = View::make('polls3.poll_body', compact('tokenEncript', 'id_user'));
            return $view;
        }
    }
    ...
    public function contenFormPoll(request $request)
    {
        ...
        $Selectores = $this->formularioEncuesta('matriz3', '1');
        $SelectoresDato = $this->formularioEncuesta('matriz3', '0');
        $selectGet = $this->selectFirst('encuesta3', 'id_usuario', $id_user);
        $magnitud = $this->selectorGeneric('magnitud', 'id_magnitud', 'descripcion');
        $view = View::make('polls3.poll_body_form', compact('tokenEncript', 'Selectores', 'SelectoresDato', 'id_user','selectGet', 'magnitud'));
