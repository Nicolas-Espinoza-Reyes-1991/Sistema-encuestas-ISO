...
class PollsController extends Controller
{
    ...
    public function poll_body($tokenEncript){
        ...
        if ($confirmadato != null || $confirmadato != '') { 
            $view = View::make('polls.poll_body', compact('tokenEncript', 'id_user'));
            return $view;
        }
    }
    ...
    public function contenFormPoll(request $request)
    {
        ...
        $Selectores = $this->formularioEncuesta('matriz', '1');
        $SelectoresDato = $this->formularioEncuesta('matriz', '0');
        $selectGet = $this->selectFirst('encuesta', 'id_usuario', $id_user);
        $view = View::make('polls.poll_body_form', compact('tokenEncript', 'Selectores', 'SelectoresDato', 'id_user', 'selectGet'));
